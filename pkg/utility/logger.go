...
// IsInitialized checks if the logger is properly initialized
...
func NewLogger() *Logger {
	...
	if err != nil {
		...
		_, err := log.ReadFile(logSettingsPath)
		if err != nil {
			fmt.Println("Failed to load log configuration, using default settings")

			// Initialize a console logger as a fallback
			log.Global.AddFilter("stdout", log.INFO, log.NewConsoleLogWriter())

			return &Logger{
				logger: log.Global["stdout"],
			}
		}
		folder = "../logs"
	}
	...
	return &Logger{
		logger: log.LOGGER("fileLogs"), // Ensure this matches your log config file
	}
}
